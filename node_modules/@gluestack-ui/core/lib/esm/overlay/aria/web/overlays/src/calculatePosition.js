...
export function calculatePosition(opts) {
    let { placement, targetNode, targetRect, overlayNode, scrollNode, padding, shouldFlip, boundaryElement, offset, crossOffset, shouldOverlapWithTrigger, } = opts;
    ...
    if (targetRect) {
        childOffset = isBodyContainer
            ? targetRect
            : Object.assign(Object.assign({}, targetRect), { top: targetRect.top - getOffset(container).top, left: targetRect.left - getOffset(container).left });
    }
    else {
        childOffset = isBodyContainer
